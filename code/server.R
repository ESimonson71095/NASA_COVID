...
StateNames <- c("Florida", "Georgia", "Mississippi","Alabama","Louisiana", "Colorado", "California", "Washington")
...
for(landowner in 1:2){
  ...
  #Read in MODIS
  if(landowner == 1){
    af.viirs <- read.csv(paste0(fdir,"VIIRS_AF_Weekly_TimeSeries_US_Fedlands.csv"), sep = "")
    owner <- "FED"
  }
  if(landowner == 2){
    af.viirs <- read.csv(paste0(fdir,"VIIRS_AF_Weekly_TimeSeries_US_States.csv"), sep = "")
    owner <- "ALL"
  }
...
shinyServer(function(input, output) {
 ...
 output$MCD64A1_Land_Type <- renderUI({
   selectInput("MCD64A1_SelectedLand","Federal (1) or Statewide (2):", LandType)
 })
...
# MCD64A1_TimeSeries_Chart <- reactive({
#   dataframe <- modis.ba[LT(),ss(),,]
#   colnames(dataframe) <- c("2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011",
#                            "2012","2013","2014","2015","2016","2017","2018","2019","2020")
# })
 
output$dto <- renderTable(modis.ba[LT(),ss(),,]) #, rownames = c("2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011",
                                                 #            "2012","2013","2014","2015","2016","2017","2018","2019","2020"))
...
# Plot time series of AF
 plotInputAF <- function(){
    ...
    legend(0,max(modis.af[LT2(), ss2(),,],na.rm=T),c(2001:2019),col=rev(heat.colors(20)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
    legend(0,max(modis.af[LT2(), ss2(),,],na.rm=T)*.7,c(2020),col=3,lwd=2,bty="n",cex=0.7)
    legend(0,max(modis.af[LT2(), ss2(),,],na.rm=T)*.65,"mean",col=1,lwd=2,bty="n",cex=0.7)
...
# Plot cumulative AF
 plotInputAF2 <- function(){
    ...
    legend(0,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T)),c(2001:2019),col=rev(heat.colors(19)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
    legend(0,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T))*0.7,c(2020),col=3,lwd=2,bty="n",cex=0.7)
    legend(0,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T))*0.65,"mean",col=1,lwd=2,bty="n",cex=0.7)
 }
...
output$ViirsActiveFire_Land_Type <- renderUI({
  selectInput("SelectedLand3","Federal (1) or Statewide (2):", LandType)
})
...
plotInputViirsAF <- function(){
  ...
  viirs.af[LT3(),ss3(),42:52,9] <- NA
  plot(0,0,xlim=c(0,53),ylim=c(1,max(viirs.af[LT3(),ss3(),,],na.rm=T)),col="white",xlab="Weeks Since Jan 1", ylab="VIIRS Weekly Fire Counts (#)",main=paste(StateNames[ss3()]))
  ...
  legend(0,max(viirs.af[LT3(), ss3(),,],na.rm=T),c(2012:2019),col=rev(heat.colors(8)),ncol=3,lwd=2,bty="n",cex=0.8)
  legend(0,max(viirs.af[LT3(), ss3(),,],na.rm=T)*0.87,c(2020),col=3,lwd=2,bty="n",cex=0.8)
  legend(0,max(viirs.af[LT3(), ss3(),,],na.rm=T)*0.83,"mean",col=1,lwd=2,bty="n",cex=0.8)
...
plotInputViirsAF2 <- function(){
  ...
  plot(0,0,xlim=c(0,53),ylim=c(1,max(colSums(viirs.af[LT3(),ss3(),,],na.rm=T))),col="white",xlab="Weeks Since Jan 1", ylab="VIIRS Weekly Fire Counts (#)",main=paste(StateNames[ss3()]))
  for(i in 1:length(2012:2019)){
  ...
  lines(1:42, cumsum(viirs.af[LT3(),ss3(),1:42,9]),lwd=3,col=3)
  legend(1,max(colSums(viirs.af[LT3(),ss3(),,],na.rm=T)),c(2012:2019),col=rev(heat.colors(8)),ncol=3,lwd=2,bty="n",cex=0.8)
