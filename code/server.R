...
modis.af <- array(0,c(2,length(StateNames),52,length(2001:2021)))
modisAF.years <- 2001:2021
modisAF.weeks <- 1:52
...
modisAF.endweek <- length(which(!is.na(modis.af[1,5,,21])))

# Create dimnames for the table so that columns and rows are named when CSV is exported
dimnames(modis.af) <- list(c("Statewide", "Federal"),c(),1:52,2001:2021)
...
viirs.af <- array(0,c(2,length(StateNames),52,length(2012:2021)))
viirsAF.years <- 2012:2021
viirsAF.weeks <- 1:52
...
# Remove Last Weeks of 2020 w/o data
viirsAF.endweek <- length(which(!is.na(viirs.af[1,5,,10]))) # [AllLand, California, ,2020]

# Create dimnames for the table so that columns and rows are named when CSV is exported
dimnames(viirs.af) <- list(c("Statewide", "Federal"),c(),1:52,2012:2021)
...
# Plot time series of AF
 plotInputAF <- function(){
    ...
    lines(1:52,modis.af[LT2(),ss2(),,20],lwd=3,col=rev(heat.colors(20))[20],lty=2)
    lines(1:52,modis.af[LT2(),ss2(),,21],lwd=3,col=3)
    #lines(1:52,modis.af[LT2(),ss2(),,21],lwd=3,col=4)
    legend(1,max(modis.af[LT2(), ss2(),,],na.rm=T),c(2001:2020),col=rev(heat.colors(20)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
    legend(1,max(modis.af[LT2(), ss2(),,],na.rm=T)*.7,c(2021),col=3,lwd=2,bty="n",cex=0.7)
    #legend(1,max(modis.af[LT2(), ss2(),,],na.rm=T)*.65,c(2021),col=4,lwd=2,bty="n",cex=0.7)
...
# Plot cumulative AF
 plotInputAF2 <- function(){
    ...
    for(i in 1:length(2001:2020)){
        lines(1:52, cumsum(modis.af[LT2(), ss2(),,i]),col=rev(heat.colors(21))[i])
    }
    lines(1:52, cumsum(rowMeans(modis.af[LT2(),ss2(),,],na.rm=T)),lwd=3,col=1)
    lines(1:52, cumsum(modis.af[LT2(),ss2(),,20]),lwd=3,col=rev(heat.colors(21))[21],lty=2)
    lines(1:modisAF.endweek, cumsum(modis.af[LT2(),ss2(),1:modisAF.endweek,21]),lwd=3,col=3)
    legend(1,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T)),c(2001:2020),col=rev(heat.colors(20)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
    legend(1,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T))*0.7,c(2021),col=3,lwd=2,bty="n",cex=0.7)
...
plotInputViirsAF <- function(){
  ...
  lines(1:52,viirs.af[LT3(),ss3(),,9],lwd=3,col=rev(heat.colors(20))[20],lty=2)
  lines(1:52,viirs.af[LT3(),ss3(),,10],lwd=3,col=3)
  legend(1,max(viirs.af[LT3(), ss3(),,],na.rm=T),c(2012:2020),col=rev(heat.colors(9)),lty=c(rep(1,8),2),ncol=3,lwd=2,bty="n",cex=0.8)
  legend(1,max(viirs.af[LT3(), ss3(),,],na.rm=T)*0.85,c(2021),col=3,lwd=2,bty="n",cex=0.8)
  legend(1,max(viirs.af[LT3(), ss3(),,],na.rm=T)*0.80,"mean",col=1,lwd=2,bty="n",cex=0.8)
...
plotInputViirsAF2 <- function(){
  ...
  lines(1:viirsAF.endweek, cumsum(viirs.af[LT3(),ss3(),1:viirsAF.endweek,10]),lwd=3,col=3)
  legend(1,max(colSums(viirs.af[LT3(),ss3(),,],na.rm=T)),c(2012:2020),col=rev(heat.colors(9)),lty=c(rep(1,8),2),ncol=3,lwd=2,bty="n",cex=0.8)
  legend(1,max(colSums(viirs.af[LT3(),ss3(),,],na.rm=T))*.85,c(2021),col=3,lwd=2,bty="n",cex=0.8)
  legend(1,max(colSums(viirs.af[LT3(),ss3(),,],na.rm=T))*.80,"mean",col=1,lwd=2,bty="n",cex=0.8)
}
