...
dimnames(modis.ba) <- list(c(),c(),1:12,2000:2020)

#####################################################################################################################################################

# DATA WRANGLING FOR MODIS ACTIVE FIRE
fdir <- "www/"
fdirM <- "F:/ESimonson_Workspace/NASA_Fire_Project/Fire-COVID19-ES/data/ActiveFire/MODIS/"
...
for(landowner in 1:2){
   ...
   #Read in MODIS
   if(landowner == 1){
      af.modis <- read.csv(paste0(fdirM,"MODIS_AF_Weekly_TimeSeries_US_Fedlands_R.csv"), sep = "")
      owner <- "FED"
   }
   if(landowner == 2){
      af.modis <- read.csv(paste0(fdirM,"MODIS_AF_Weekly_TimeSeries_US_States_R.csv"), sep = "")
      owner <- "ALL"
   }
...
dimnames(modis.af) <- list(c(),c(),1:52,2001:2020)
...
#Remove Jul-Dec 2020
viirsAF.endweek <- length(which(!is.na(viirs.af[2,5,,9])))
#viirs.af[,,(viirsAF.endweek+1):52,9] <- NA

dimnames(viirs.af) <- list(c(),c(),1:52,2012:2020)
...
#output$dto <- renderTable(modis.ba[LT(),ss(),,])
...
# Plot time series of AF
 plotInputAF <- function(){
    ...
    modis.af[LT2(),ss2(),modisAF.endweek:52,20] <- NA
    ...
    legend(1,max(modis.af[LT2(), ss2(),,],na.rm=T),c(2001:2019),col=rev(heat.colors(19)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
...
# Plot cumulative AF
 plotInputAF2 <- function(){
    ...
    legend(1,max(colSums(modis.af[LT2(), ss2(),,],na.rm=T)),c(2001:2019),col=rev(heat.colors(19)),lty=c(rep(1,19),2),ncol=3,lwd=2,bty="n",cex=0.7)
...
plotInputViirsAF <- function(){
  ...
  viirs.af[LT3(),ss3(),viirsAF.endweek:52,9] <- NA
  ...
  legend(1,max(viirs.af[LT3(), ss3(),,],na.rm=T),c(2012:2019),col=rev(heat.colors(8)),ncol=3,lwd=2,bty="n",cex=0.8)
